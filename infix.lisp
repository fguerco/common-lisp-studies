...
(defun decompose (op expr)
  (cond ((and (consp expr) (cddr expr))
...
(defun invert-signals (expr)
  (reduce (lambda (it prev)
            (cond ((consp it) (cons (invert-signals it) prev))
                  ((and (eql :invert it) (atom (car prev))) (list* -1 '* prev))
                  ((and (eql :invert it) (consp (car prev)))
                   (list (list -1 '* (car prev))))
                  (t (cons it prev))))
...
(defmacro make-assertions (test-data)
  `(progn
     ,@(mapalist (lambda (expr expected)
                   `(is = ,expected (infix ,expr)))
                 test-data)))
